...
class Eng_GUI :
    def __init__(self, master) -> object:
        ...
        master.title("Flash Cards learn English Vocabulary")
        master.config(padx=50, pady=50, bg=BACKGROUND_COLOR)
        self.current_card = {}
        self.to_learn = {}
        try:
            self.data_file = pd.read_csv("./data/Eng_words_to_learn.csv")
        except FileNotFoundError:
            self.original_data = pd.read_csv("./data/english_word.csv")
    ...
    def next_card(self):
        ...
        self.card.itemconfig(self.card_title, text="English", fill="black")
        self.card.itemconfig(self.card_word, text= self.current_card["English"], fill="black")
    ...
    def flip_card(self):
        ...
        self.card.itemconfig(self.card_title, text="VietNamese", fill="white")
        self.card.itemconfig(self.card_word, text=self.current_card["VietNamese"], fill="white")
